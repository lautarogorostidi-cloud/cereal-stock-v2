...
export default function SidebarAgroquimicos({ perfil }: SidebarProps) {
    ...
    <aside className="w-60 flex flex-col bg-campo-950 text-campo-100 shrink-0">
      ...
      <div className="px-5 py-5 border-b border-campo-800">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <div className="text-campo-400 text-xs leading-tight">Agroquímicos</div>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-2 space-y-0.5 overflow-y-auto">
        {nav.map(item => {
          const active =
            pathname === item.href ||
            (item.href !== '/agroquimicos' && pathname.startsWith(item.href))
          return (
            <Link
              key={item.href}
              href={item.href}
