...
const nav = [
  { href: '/combustible',             label: 'Dashboard',   icon: '📊' },
  { href: '/combustible/stock',       label: 'Stock',       icon: '⛽' },
  { href: '/combustible/movimientos', label: 'Movimientos', icon: '📋' },
  { href: '/combustible/maquinas',    label: 'Maquinaria',  icon: '🚜' },
  { href: '/combustible/tanques',     label: 'Tanques',     icon: '🛢️' },
  { href: '/combustible/reportes',    label: 'Reportes',    icon: '📈' },
]

interface SidebarProps { perfil: Perfil | null }

export default function SidebarCombustible({ perfil }: SidebarProps) {
    ...
    <aside className="w-60 flex flex-col bg-campo-950 text-campo-100 shrink-0">
      ...
      <div className="px-5 py-5 border-b border-campo-800">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-emerald-700 flex items-center justify-center text-base">⛽</div>
          <div>
            <div className="font-bold text-white text-sm leading-tight">Campo</div>
            <div className="text-campo-400 text-xs leading-tight">Combustible</div>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-2 space-y-0.5 overflow-y-auto">
        {nav.map(item => {
          const active =
            pathname === item.href ||
            (item.href !== '/combustible' && pathname.startsWith(item.href))
          return (
            <Link
              key={item.href}
              href={item.href}
