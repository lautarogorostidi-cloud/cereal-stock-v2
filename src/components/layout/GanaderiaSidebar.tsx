...
const nav = [
  { href: '/ganaderia',                label: 'Hacienda',          icon: '🐄' },
  { href: '/ganaderia/feedlot',        label: 'Feedlot',           icon: '🐮' },
  { href: '/ganaderia/costos-sanidad', label: 'Costos y Sanidad',  icon: '💉' },
  { href: '/ganaderia/resumen',        label: 'Resumen campaña',   icon: '📋' },
  ...
  return (
    <aside className="w-60 flex flex-col bg-campo-950 text-campo-100 shrink-0">
      <div className="px-5 py-5 border-b border-campo-800">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <div className="text-campo-400 text-xs leading-tight">Ganaderia</div>
          </div>
        </div>
      </div>

      <div className="px-3 pt-3">
        <Link href="/selector" className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-campo-400 hover:text-white hover:bg-campo-800 transition-colors">
      ...
      <nav className="flex-1 px-3 py-2 space-y-0.5 overflow-y-auto">
        {nav.map(item => {
          const active = item.href === '/ganaderia' ? pathname === '/ganaderia' : pathname.startsWith(item.href)
          return (
            <Link key={item.href} href={item.href}
              className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors ${active ? 'bg-campo-700 text-white font-medium' : 'text-campo-300 hover:bg-campo-800 hover:text-white'}`}>
      ...
      <div className="px-3 py-4 border-t border-campo-800">
        ...
        <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-campo-400 hover:text-white hover:bg-campo-800 transition-colors">
