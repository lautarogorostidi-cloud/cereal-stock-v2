...
export default function EditarContratoPage() {
      ...
      if (numero) {
        ...
        if (contrato) {
          ...
          setForm({
            ...
            cuit_corredor: contrato.cuit_corredor ?? '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const opcionales: Record<string, any> = {
      nro_operacion_corredor: form.nro_operacion_corredor,
      cuit_comprador: form.cuit_comprador,
      corredor_nombre: form.corredor,
      corredor_cuit: form.cuit_corredor,
      sucursal: form.sucursal,
      calidad_producto: form.calidad_producto,
      condicion_entrega: form.condicion_entrega,
      pct_condicion: form.pct_condicion,
      precio_unitario: form.precio_unitario ? parseFloat(form.precio_unitario) : null,
      puerto_id: form.puerto_id || null,
      acopio_id: form.acopio_id || null,
      procedencia: form.procedencia,
      destino: form.destino,
      fecha_inicio_entrega: form.fecha_inicio_entrega || null,
      fecha_fin_entrega: form.fecha_fin_entrega || null,
      plazo_fijacion: form.plazo_fijacion,
      fecha_cobro_corredor: form.fecha_cobro_corredor || null,
      pago_vendedor_dias: form.pago_vendedor_dias,
      documentacion: form.documentacion,
      nro_sio_granos: form.nro_sio_granos,
      cupos: form.cupos,
      cupos_asignados: form.cupos_asignados,
      comision_corredor: form.comision_corredor ? parseFloat(form.comision_corredor) : null,
      comision_corredor_monto: form.comision_corredor_monto ? parseFloat(form.comision_corredor_monto) : null,
      condiciones: form.condiciones,
      observaciones: form.observaciones,
    }

    Object.entries(opcionales).forEach(([k, v]) => { if (v !== '' && v !== undefined) payload[k] = v })
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-campo-500 text-sm">Modificá los datos del contrato {numero}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-campo-700 mb-1">Plazo de fijación</label><input value={form.plazo_fijacion} onChange={e => set('plazo_fijacion', e.target.value)} className="input-field" /></div>
            ...
            <div><label className="block text-sm font-medium text-campo-700 mb-1">Pago al vendedor</label><input value={form.pago_vendedor_dias} onChange={e => set('pago_vendedor_dias', e.target.value)} placeholder="1 día más" className="input-field" /></div>
            <div><label className="block text-sm font-medium text-campo-700 mb-1">Documentación</label><input value={form.documentacion} onChange={e => set('documentacion', e.target.value)} className="input-field" /></div>
            ...
            <div><label className="block text-sm font-medium text-campo-700 mb-1">Condición entrega</label><input value={form.condicion_entrega} onChange={e => set('condicion_entrega', e.target.value)} placeholder="puesto sobre camión" className="input-field" /></div>
          </div>
        </div>
