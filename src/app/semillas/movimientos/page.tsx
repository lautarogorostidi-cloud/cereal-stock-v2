...
type Movimiento = {
  ...
  semillas_productos: { nombre: string; unidad: string; marca: string | null; cultivos: { nombre: string } | null } | null
...
export default function MovimientosSemillasPage() {
  ...
  async function cargar() {
    ...
    const { data, error } = await supabase
      .from('semillas_movimientos')
      .select('*, semillas_productos(nombre, unidad, marca, cultivos(nombre)), proveedores(nombre)')
  ...
  const UNIDADES_PRODUCTO = ['bolsas', 'kg', 'u']

  async function handleGuardarNuevoProducto() {
    if (!nuevoProducto.nombre || !nuevoProducto.cultivo_id) return
    setSavingProducto(true)
    setErrorProducto(null)
    const { data, error } = await supabase.from('semillas_productos').insert({
  ...
  async function handleGuardar() {
    ...
    if (nuevoProductoMode) {
      setError('Primero guardá la nueva semilla o cancelá.')
      return
    }
  ...
  async function handleBorrar(id: number) {
    ...
    await supabase.from('semillas_movimientos').delete().eq('id', id)
    cargar()
  }
  ...
  const movFiltrados = movimientos
    ...
    .filter(m => {
      ...
      return (
        m.semillas_productos?.nombre?.toLowerCase().includes(q) ||
        ...
        <div className="card p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="lg:col-span-2">
              <label className="block text-xs font-medium text-campo-700 mb-1">Semilla *</label>
              {!nuevoProductoMode ? (
                <>
                  <select value={form.producto_id} onChange={e => {
                    ...
                    <option value="__nuevo__">➕ Agregar nueva semilla...</option>
                  </select>
                </>
              ) : (
                <div className="space-y-2 p-3 rounded-lg border border-emerald-200 bg-emerald-50">
                  <div className="text-xs font-medium text-emerald-700 mb-2">Nueva semilla</div>
                  <div className="grid grid-cols-2 gap-2">
                    <input type="text" value={nuevoProducto.nombre} onChange={e => setNuevoProducto(p => ({ ...p, nombre: e.target.value }))}
                      placeholder="Nombre / híbrido *" className="rounded-lg border border-campo-200 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400" />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-campo-100 bg-campo-50">
                ...
                <th className="text-left px-4 py-3 font-semibold text-campo-700">Semilla</th>
              ...
              {movFiltrados.map((m, i) => (
                <tr key={i} className="border-b border-campo-50 hover:bg-campo-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="font-medium text-campo-900">{m.semillas_productos?.nombre ?? '—'}</div>
                    {m.semillas_productos?.cultivos?.nombre && <div className="text-xs text-campo-400">{m.semillas_productos.cultivos.nombre}</div>}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-right font-medium text-campo-900">
                    {fmt(m.cantidad)} <span className="text-xs text-campo-400">{m.semillas_productos?.unidad}</span>
                  </td>
